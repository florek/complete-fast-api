# Quiz FastAPI – 12.02.2026

1. **[łatwe]** W tym projekcie endpoint GET /user/ zwraca listę użytkowników. Skąd klient wie, że odpowiedź nie zawiera pól hasła?
   - A) db_user.get_all_users() zwraca tylko username i email
...
6. **[średnie]** W endpoincie create_comment występują: blog (BlogModel), id, comment_id (Path), content (Body), v (Query). Skąd FastAPI wie, skąd brać każdą wartość?
   ...
   - D) Wszystko oprócz id i comment_id pochodzi z body JSON

7. **[średnie]** W relacji DbArticle ma user_id = Column(Integer, ForeignKey('users.id')) oraz user = relationship('DbUser', back_populates='items'). Co robi samo ForeignKey?
   - A) Tworzy w Pythonie atrybut article.user
   - B) Tworzy w bazie klucz obcy i integralność referencyjną; relationship służy do dostępu w kodzie
   - C) Automatycznie ładuje użytkownika przy każdym odczycie
   - D) Zastępuje osobną tabelę użytkowników
...
12. **[trudne]** Wywołanie models.Base.metadata.create_all(bind=engine) przy starcie aplikacji:
    - A) Tworzy tabele użytkowników i artykułów
...
14. **[średnie]** W logice delete_user przed db.delete(db_user) jest sprawdzenie if not db_user: return None. Po co?
    - A) Aby zwrócić 204 No Content
    - B) Przekazanie None do db.delete() powoduje błąd (np. UnmappedInstanceError); trzeba sprawdzić istnienie obiektu
    - C) SQLAlchemy wymaga db.refresh() przed delete
    - D) db.delete() wymaga podania typu przy braku obiektu
...
20. **[trudne]** Przy aktualizacji użytkownika w db_user zmiany wprowadza się przez przypisanie pól (db_user.username = ...), a nie przez metodę update(). Dlaczego?
